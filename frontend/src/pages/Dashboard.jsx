...
import { Play, Pause, Circle, Square, Zap, Thermometer, Activity, Settings, Wifi, WifiOff, RefreshCcw } from 'lucide-react';
...
export default function Dashboard({ socket }) {
    ...
    const toggleRecording = () => {
        triggerHaptic();
        if (!isLogging) {
            // Send user ID for DB association
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-24 lg:pb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                <div className="bg-[#161b22] border border-gray-800 rounded-2xl p-8 flex flex-col items-center justify-center relative shadow-2xl overflow-hidden group">
                    ...
                    <h1 className={`text-8xl font-mono font-black tracking-tighter transition-all duration-300 ${mode === 'A' ? 'text-blue-400' : 'text-emerald-400'}`}>
                        {reading}
                    </h1>
                ...
                <div className="p-4 bg-[#1c2128] border-b border-gray-800 flex flex-wrap justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={toggleRecording} title="Record Session"
                            className={`p-2 rounded-xl transition-all border ${isLogging ? 'bg-red-500 border-red-400 text-white shadow-[0_0_15px_rgba(239,68,68,0.4)]' : 'bg-gray-800 border-gray-700 text-gray-400 hover:border-red-500/50'}`}>
                            {isLogging ? <Square size={16} fill="white" /> : <Circle size={16} fill="currentColor" />}
                        </button>

                        <div className="flex items-center gap-3 bg-[#0d1117] px-4 py-2 rounded-xl border border-gray-700">
                            ...
                            <span className="text-amber-500 font-mono text-xs w-8">{triggerLevel}V</span>
                        </div>
